...
enriched = (
    txn_cols
    .join(F.broadcast(regions), on="region_id", how="left")
...
monthly_summary = (
    enriched
    .groupBy("customer_id", "region_id", "month", "tier")
    .agg(
...
try:
    monthly_summary.write.mode("overwrite").partitionBy("month", "tier").parquet(
