...
active_products = (
    orders
    .filter(F.col("sale_date") >= F.lit("2024-01-01"))
    .select("product_id")
    .distinct()
)
...
try:
    top_products.write.mode("overwrite").parquet(f"{OUTPUT_ROOT}/top_products")
    monthly_rollup.write.mode("overwrite").partitionBy("month").parquet(
        f"{OUTPUT_ROOT}/monthly_rollup"
    )
    active_products.write.mode("overwrite").parquet(f"{OUTPUT_ROOT}/active_products")
