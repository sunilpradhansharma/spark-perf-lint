...
spark = (
    SparkSession.builder.appName("nightly_ml_scoring")
...
candidates = features.filter(F.col("is_eligible") == True).join(  # noqa: E712
    model_meta, on="model_id", how="inner"
)
...
scored = candidates.withColumn(
    "raw_score", F.col("feature_1") * 0.4 + F.col("feature_2") * 0.6
).withColumn(
    "score_bucket",
...
# hundreds of cores.
score_distribution = scored.groupBy("model_id", "score_bucket").agg(
    F.count("*").alias("count"), F.avg("raw_score").alias("avg_score")
)
